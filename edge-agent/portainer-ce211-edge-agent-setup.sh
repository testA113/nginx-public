...
main() {
  ...
  curl -L https://raw.githubusercontent.com/testA113/nginx-public/main/edge-agent/portainer-agent-edge-k8s.yaml -o portainer-agent-edge-k8s.yaml || errorAndExit "Unable to download agent manifest"
  ...
  kubectl create secret generic portainer-agent-edge-key --from-literal="edge.key=$EDGE_KEY" --from-literal="edge.secret=$AGENT_SECRET"  -n portainer
